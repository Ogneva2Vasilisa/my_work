...
string passs = "___________________________";
...
int prov_mac_addr()
{
	...
	if ((dwRetVal = GetAdaptersInfo(pAdapterInfo, &ulOutBufLen)) == NO_ERROR) {
		...
		while (pAdapter) {
			...
			//printf("%s %d\n", mac, strncmp(mac, "00:05:69", 8));
			if (!strncmp(mac, "00:05:69", 8) || // VMware
				!strncmp(mac, "00:0C:29", 8) || // VMware
				!strncmp(mac, "00:50:56", 8) || // VMware
				!strncmp(mac, "00:1C:14", 8) || // VMware
				!strncmp(mac, "00:1C:14", 8) || // VirtualBox
				!strncmp(mac, "08:00:27", 8)) { // VirtualBox
				printf("WM found. Exit.");
				exit(0);
			}
...
BOOL CALLBACK EnumWindowsProc(HWND hwnd, LPARAM lParam) {
	...
	// Проверка на частичное совпадение заголовка
	std::string windowTitle = title;
	if (windowTitle.find("IDA") != std::string::npos) {
		foundHwnd = hwnd; // Сохраняем найденный дескриптор
...
int check_pas_from_file(char* u) {
	...
	EnumWindows(EnumWindowsProc, NULL); // Начинаем перечисление окон
	if (foundHwnd != NULL) {
		std::cout << "turn off IDA" << std::endl;
		exit(-1);
	};
...
int main() {
	...
	prov_mac_addr();
		...
		int condition = 1; // Условие всегда истинное

		if (condition) {
			goto execute_protected;
		}
		else {
			goto execute_dummy;
		}
