...
// Таблица символов для переменных программы ($A, $B, $Var1, ...)
...
// Таблица символов для переменных полиномов (x, y, x1, x2, ...)
...
int poly_var_count = 0;  // Счетчик определенных переменных
...
{
       0,    85,    85,    86,    86,    90,    99,   115,   134,   153,
     169,   172,   175,   178,   184,   187,   190,   193,   199,   202,
     205,   208,   211
};
#endif
...
static const char *const yytname[] =
{
  ...
  "IDENTIFIER", "ASSIGN", "LPAREN", "RPAREN", "END_LINE", "PRINT", "PLUS",
  "MINUS", "MULTIPLY", "DIVIDE", "NEG", "POW", "$accept", "program",
...
{
     -13,    19,   -13,   -13,     3,     5,    29,   -13,    29,    29,
       9,    -9,     1,    11,    29,    -2,   -13,   -13,    40,    -9,
     -13,   -13,    39,    39,    39,    39,    11,     4,    -9,   -13,
      22,   -13,     1,     1,    11,    11,   -13,   -13
};
...
{
       2,     0,     1,    20,    22,    21,     0,     4,     0,     0,
       0,     5,    13,    17,     0,     0,    22,    21,     0,     9,
      12,     3,     0,     0,     0,     0,    15,     0,     6,     7,
       0,    19,    10,    11,    14,    16,    18,     8
};
...
{
      26,    29,    22,    23,     3,    16,    17,    36,     6,    14,
      30,    15,    34,    35,    24,    25,    32,    33,    21,     2,
      26,    26,     3,     4,     5,    37,     6,    27,     7,     8,
       0,     9,     3,    16,    17,     0,     6,    18,     0,    19,
      20,     9,     3,    16,    17,    28,     6,     0,    31,     0,
       0,    22,    23
};

static const yytype_int8 yycheck[] =
{
      12,     3,    11,    12,     3,     4,     5,     3,     7,     6,
      12,     6,    24,    25,    13,    14,    22,    23,     9,     0,
      32,    33,     3,     4,     5,     3,     7,    16,     9,    10,
      -1,    12,     3,     4,     5,    -1,     7,     6,    -1,     8,
       9,    12,     3,     4,     5,    14,     7,    -1,     8,    -1,
      -1,    11,    12
};
...
{
       0,    18,     0,     3,     4,     5,     7,     9,    10,    12,
      19,    20,    21,    22,     6,     6,     4,     5,    20,    20,
      20,     9,    11,    12,    13,    14,    22,    16,    20,     3,
      12,     8,    21,    21,    22,    22,     3,     3
};
...
#line 90 "gram.y"
        ...
        print_polynomial((yyvsp[0].poly));  
        int value = evaluate_polynomial((yyvsp[0].poly));
        //if (value != INT_MIN) {
        //    printf("Line %d: value of polynom %d\n", yylineno-1, value);
        //}
        free_polynomial((yyvsp[0].poly)); 
    }
#line 1170 "gram.tab.c"
    break;

  case 6: /* line: VAR ASSIGN expr  */
#line 99 "gram.y"
                      { 
        ProgramVar *var = find_program_var((yyvsp[-2].str));
        if (var) {
            // Обновляем существующую переменную
            free_polynomial(var->value);
            var->value = (yyvsp[0].poly);
        } else if (program_var_count < 100) {
            // Добавляем новую переменную
            strncpy(program_vars[program_var_count].name, (yyvsp[-2].str), 31);
            program_vars[program_var_count].value = (yyvsp[0].poly);
            program_var_count++;
        } else {
...
#line 1191 "gram.tab.c"
    break;

  case 7: /* line: IDENTIFIER ASSIGN NUMBER  */
#line 115 "gram.y"
                               { 
        // Поиск существующей переменной
        bool found = false;
        for (int i = 0; i < poly_var_count; i++) {
            if (strcmp(poly_vars[i].name, (yyvsp[-2].str)) == 0) {
                poly_vars[i].value = (yyvsp[0].num);
        ...
        if (!found && poly_var_count < 100) {
            strncpy(poly_vars[poly_var_count].name, (yyvsp[-2].str), 15);
            poly_vars[poly_var_count].value = (yyvsp[0].num);
            poly_vars[poly_var_count].is_defined = true;
            poly_var_count++;
        }
    }
#line 1215 "gram.tab.c"
    break;

  case 8: /* line: IDENTIFIER ASSIGN MINUS NUMBER  */
#line 134 "gram.y"
                                     { 
        // Поиск существующей переменной
        bool found = false;
        for (int i = 0; i < poly_var_count; i++) {
            if (strcmp(poly_vars[i].name, (yyvsp[-3].str)) == 0) {
                poly_vars[i].value = -(yyvsp[0].num);
                poly_vars[i].is_defined = true;
                found = true;
                break;
        ...
        // Если не найдена, добавляем новую
        if (!found && poly_var_count < 100) {
            strncpy(poly_vars[poly_var_count].name, (yyvsp[-3].str), 15);
            poly_vars[poly_var_count].value = -(yyvsp[0].num);
            poly_vars[poly_var_count].is_defined = true;
            poly_var_count++;
        }
    }
#line 1239 "gram.tab.c"
    break;

  case 9: /* line: PRINT expr  */
#line 153 "gram.y"
                 { 
        //printf("Line %d: ", yylineno-1); 
        //print_polynomial($2); 
...
#line 1257 "gram.tab.c"
    break;

  case 10: /* expr: expr PLUS term  */
#line 169 "gram.y"
                            { 
        printf("        expr:expr PLUS term\n");
        (yyval.poly) = add_polynomials((yyvsp[-2].poly), (yyvsp[0].poly)); free_polynomial((yyvsp[-2].poly)); free_polynomial((yyvsp[0].poly)); }
#line 1265 "gram.tab.c"
    break;

  case 11: /* expr: expr MINUS term  */
#line 172 "gram.y"
                            { 
        printf("        expr:expr MINUS term\n");
        (yyval.poly) = subtract_polynomials((yyvsp[-2].poly), (yyvsp[0].poly)); free_polynomial((yyvsp[-2].poly)); free_polynomial((yyvsp[0].poly)); }
...
#line 175 "gram.y"
                            { 
        printf("        expr:MINUS expr prec NEG\n");
        (yyval.poly) = negate_polynomial((yyvsp[0].poly)); }
#line 1281 "gram.tab.c"
    break;
...
#line 1289 "gram.tab.c"
    break;

  case 14: /* term: term MULTIPLY factor  */
#line 184 "gram.y"
                            { 
        printf("    term:term MULTIPLY factor \n");
        (yyval.poly) = multiply_polynomials((yyvsp[-2].poly), (yyvsp[0].poly)); free_polynomial((yyvsp[-2].poly)); free_polynomial((yyvsp[0].poly)); }
#line 1297 "gram.tab.c"
    break;
...
#line 187 "gram.y"
        ...
        (yyval.poly) = multiply_polynomials((yyvsp[-1].poly), (yyvsp[0].poly)); free_polynomial((yyvsp[-1].poly)); free_polynomial((yyvsp[0].poly)); }
#line 1305 "gram.tab.c"
    break;

  case 16: /* term: term DIVIDE factor  */
#line 190 "gram.y"
                            { 
        printf("    term:term DIVIDE factor\n");
        printf("[WARNING] Line %d: division is not provided in calculator. The result may be incorrect!\n", yylineno - 1); }
...
#line 193 "gram.y"
        ...
        printf("    term:factor\n");
        (yyval.poly) = (yyvsp[0].poly); }
#line 1321 "gram.tab.c"
    break;

  case 18: /* factor: factor POW NUMBER  */
#line 199 "gram.y"
                            { 
        printf("factor:factor POW NUMBER\n");
    (yyval.poly) = power_polynomial((yyvsp[-2].poly), (yyvsp[0].num)); free_polynomial((yyvsp[-2].poly)); }
#line 1329 "gram.tab.c"
    break;
...
#line 205 "gram.y"
        ...
        (yyval.poly) = create_polynomial_from_number((yyvsp[0].num)); }
...
#line 208 "gram.y"
        ...
        (yyval.poly) = create_polynomial_from_identifier((yyvsp[0].str)); }
#line 1353 "gram.tab.c"
    break;
...
#line 211 "gram.y"
                            { 
        printf("factor:VAR\n");
        (yyval.poly) = get_program_var((yyvsp[0].str)); }
#line 1361 "gram.tab.c"
    break;


#line 1365 "gram.tab.c"
...
int evaluate_polynomial(const Polynomial *p) {
    ...
    for (int i = 0; i < p->mono_count; i++) {
        ...
        for (int j = 0; j < m.var_count; j++) {
            ...
            if (!var_found) {
                return INT_MIN; // Не все переменные определены
            }
        }
        result += term_value;
    }
