...
void proverka_slova_helper(char sl[15], int i, int letter, char mas[7], int kol, int st, int I, int J, int number, int num_z, int ukaz, int& masi) {
    char t = mas[letter];
    sl[i] = t;
    ...
    for (int j = 0; j < 7; j++) {
        for (int i = uk; i <= J; i++) {
            if (mas[j] != 'z' && (sl[i] == 'z' || sl[i] == 'k')) {
                if ((i == 0) && (mas[j] == 'ь'  || mas[j] == 'ы' || mas[j] == 'ъ')) {
                    break;
                }

                proverka_slova_helper(sl, i, letter, mas, kol, st, I, J, number, num_z, ukaz, masi);
...
int analise_2(char mas[15], int pos, int st_str, int ukaz, char mas_sl[7]) {
    ...
    for (int i = 1; i < 14; i++) {
        ...
        if (mas[i] == 'z' && ((mas[i - 1] != 'z' && mas[i - 1] != 'k') || (mas[i + 1] != 'z' && mas[i + 1] != 'k'))) {
    ...
    if (mas[14] == 'z' && ((mas[13] != 'z' && mas[13] != 'k'))) {
        mas[14] = 'k';
        if (mas[13] != 'z') {
            point_number[count] = 14;
            count += 1;
        }
    }
        ...
        while (T == 1 && j <= point_number[i + 1]) {
            if (mas[j] == 'z' || mas[j] == 'k') {
                ...
                while (T == 1 && j1 < 15) {
                    ...
                    int slo = 0;
                    T = 1;
                    if (mas[j1] == 'z' || mas[j1] == 'k') {
                        y++;
                        kkk = pirog(st_str, pos, j - p, j1 - 1, j1 - j + p - y - yy + 1);
                        if (kkk) {
                            for (int f = j - p; f <= j1 - 1; f++) {
                                slovas[slo] = mas[f];
                                slo += 1;
                            }
                            slovas[slo] = '\0';
                        ...
                        //int kkk = pirog(st_str, pos, j - p, j - p + slo, strlen(slovas) - y-yy);
                        //printf("%s %d\n", slovas, kkk);
                        //y<7 - ? что за условие????
                        if (kkk && y + yy - 1 < 7 && proverka_slova(slovas, 0, y + yy - 1, st_str, j - p, j - p + slo - 1, pos, 0, strlen(slovas) - (y + yy - 1), ukaz, mas_sl) == 0) {
                            //printf("%s %d\n", slovas, kkk);
                            return 10;
                        }
                    }
                ...
                if (mas[14] == 'z' || mas[14] == 'k') {
                    kkk = pirog(st_str, pos, j - p, j1 - 1, j1 - j + p - y - yy);
                    if (kkk) {
                        for (int f = j - p; f <= j1 - 1; f++) {
                            slovas[slo] = mas[f];
                            slo += 1;
                    ...
                    //int kkk = pirog(st_str, pos, j - p, j - p + slo, strlen(slovas) - y);
                    //printf("%s %d\n", slovas,kkk);
                    //y<7 - ? что за условие????
                    if (kkk && y + yy < 7 && proverka_slova(slovas, 0, y + yy, st_str, j - p, j - p + slo - 1, pos, 0, strlen(slovas) - (y + yy), ukaz, mas_sl) == 0) {
                        //printf("%s %d\n", slovas, kkk);
                        return 10;
                    }
                    j1 += 1;
...
int analyse(int ukaz, int num, char mas_sl[8]) {
    ...
    for (int i = 0; i < 15; i++) {
        ...
        if (t != 15 && analise_2(slovo, i, 0, ukaz, mas_sl) == 10 && ukaz % 2 == 1 && DOP == 0) {
            printf("1, %d\n", DOP);
            break;
        }
        cleaning_mas(slovo);
    ...
    for (int i = 0; i < 15; i++) {
        ...
        if (t != 15 && analise_2(slovo, i, 1, ukaz, mas_sl) == 10 && ukaz % 2 == 1 && DOP == 0) {
            printf("2, %d\n", DOP);
            break;
        }
        cleaning_mas(slovo);
    ...
    else if (tt == 0 && ukaz < 3 && BANK_count != 0) {
        printf("\n%s", "смена букв\n");
        change(mas_sl);
        printf("\nновые: %s\n", mas_sl);
    ...
    else if ((tt == 0 && num == 2) || BANK_count == 0 || propusk_2 > 3) {
        printf("\n%s", "конец игры\n");
        return 0;
    }
...
int main() {
    ...
    printf("\n");
    //giving_letters();
   // char c2[] = "оzкораzлzzzоzzz";
    //analise_2(c2, 0, 1,1, player_bank_1);
