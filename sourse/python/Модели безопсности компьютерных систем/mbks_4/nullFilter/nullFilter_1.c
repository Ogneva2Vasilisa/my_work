...
//         // Читаем имя процесса
//         PCHAR processEnd = strchr(currentLine, '\n');
//         if (processEnd == NULL) break;

//         PCHAR processName = ExAllocatePool2(
//             POOL_FLAG_NON_PAGED,
//             processEnd - currentLine + 1,
//             'cnfZ'
//         );

//         if (processName == NULL) {
//             KdPrint(("Memory allocation failed for process name\n"));
//             break;
//         }

//         for (INT i = 0; i < processEnd - currentLine; i++) {
//             processName[i] = currentLine[i];
//             if (processName[i] == '\r' || processName[i] == '\t') {
//                 processName[i] = '\0';
//             }
//         }
//         processName[processEnd - currentLine] = '\0';
//         processes[proccol] = processName;

//         currentLine = processEnd + 1;

//         // Обрабатываем права доступа
//         while (*currentLine != '.' && *currentLine != '\0') {
//             // Определяем тип доступа (Read/Write)
//             BOOLEAN isRead = (strncmp(currentLine, "Read", 4) == 0);
//             BOOLEAN isWrite = (strncmp(currentLine, "Write", 5) == 0);
...
//             // Пропускаем "Read " или "Write "
//             currentLine += (isRead ? 5 : 6);

//             // Читаем количество файлов
//             PCHAR numEnd = strchr(currentLine, '\n');
//             if (numEnd == NULL) break;

//             CHAR numStr[4] = { 0 };
//             for (INT i = 0; i < numEnd - currentLine; i++) {
//                 numStr[i] = currentLine[i];
//             }
//             colfiles = atoi(numStr);

//             // Выделяем память для массива файлов
//             WCHAR** filesArray = ExAllocatePool2(
//                 POOL_FLAG_NON_PAGED,
...
//             if (filesArray == NULL) {
//                 KdPrint(("Memory allocation failed for files array\n"));
//                 break;
//             }

//             if (isRead) {
//                 filesdeny[proccol] = filesArray;
//             } else {
//                 fileswrite[proccol] = filesArray;
//             }

//             currentLine = numEnd + 1;

//             // Читаем имена файлов
//             for (INT j = 0; j < colfiles; j++) {
//                 PCHAR fileEnd = strchr(currentLine, ' ');
//                 if (fileEnd == NULL) {
//                     fileEnd = strchr(currentLine, '\n');
//                 }
//                 if (fileEnd == NULL) break;

//                 CHAR filename[255] = { 0 };
//                 for (INT i = 0; i < fileEnd - currentLine; i++) {
//                     filename[i] = currentLine[i];
...
//                 ULONG size = 0;
//                 filesArray[j] = NULL;
//                 CharToWchar(filename, &filesArray[j], &size);

//                 currentLine = fileEnd + 1;
//             }
//             filesArray[colfiles] = NULL;
//         }
