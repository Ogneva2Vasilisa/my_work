...
expr:
    expr PLUS term          { 
        ...
        $$ = add_polynomials($1, $3); free_polynomial($1); free_polynomial($3); }
    | expr MINUS term       { 
        printf("        expr:expr MINUS term\n");
        $$ = subtract_polynomials($1, $3); free_polynomial($1); free_polynomial($3); }
    | MINUS expr %prec NEG  { 
...
term:
    term MULTIPLY factor    { 
        ...
        $$ = multiply_polynomials($1, $3); free_polynomial($1); free_polynomial($3); }
    | term factor           { 
        printf("    term:term factor\n");
        $$ = multiply_polynomials($1, $2); free_polynomial($1); free_polynomial($2); }
    | term DIVIDE factor    { 
...
factor:
    ...
    $$ = power_polynomial($1, $3); free_polynomial($1); }
    | LPAREN expr RPAREN    { 
        printf("factor:LPAREN expr RPAREN\n");
        $$ = $2; }
    | NUMBER                { 
        printf("factor:NUMBER\n");
        $$ = create_polynomial_from_number($1); }
    | IDENTIFIER            { 
        printf("factor:IDENTIFIER\n");
        $$ = create_polynomial_from_identifier($1); }
    | VAR                   { 
        printf("factor:VAR\n");
        $$ = get_program_var($1); }
    ;
