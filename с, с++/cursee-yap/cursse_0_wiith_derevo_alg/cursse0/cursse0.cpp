...
void proverka_slova(char sl[15], int st, int I, int J, int number, int num_n_z, int ukaz, char mas[7]) {
    ...
    short  T = 1;
    short  masi;
    short  num_z = strlen(sl) - num_n_z;
    for (char* sli = &sl[0]; sli <= &sl[J - I]; sli++) {
        if (*sli == 'z' || *sli == 'k') {
            m = mas[a[c]];
            if (m == 'z' || (p == 0 && (m == 'ъ' || m == 'ь' || m == 'ы'))) { T = 0; break; }
            *sli = m;
            if (sli - sl > 0) {
                char c0 = sl[sli - sl];
                char c1 = sl[sli - sl - 1];
                if (c0 == -72)c = 0;
                if (c1 == -72)c1 = 0;
                if ((&root_stat)->children[c0 + 32]->parent_ss[c1 + 32] == NULL) {
                    break;
                }
            }
            using_letters[c] = m;
    ...
    if (T != 0) {
        current2 = Search(sl);
        if (current2 != NULL && current2->end_of_word == true) {
            masi = massa_sl(number, I, J, sl);
            PutIn1(st, number, I, J, masi, using_letters, sl, num_n_z);
            DOP -= 1;
            if (ukaz % 2 == 1 && DOP == 0) {
                printf("DOP 2: %d\n", DOP);
                return;
    ...
    for (int i = 0; i < 7; i++) { using_letters[i] = NULL; }
    while (NextSet(7, num_z)) {
        c = 0;
        T = 1;
        strcpy(sl, sl2);
        for (char* sli = &sl[0]; sli <= &sl[J - I]; sli++) {
    ...
    for (int i = 0; i < pre_c; i++) {
        ...
        for (int j = 0; j < su_c; j++) {
            ...
            if (preffix[i][1] == suffix[j][0]) {
                ...
                if (y + yy < 7 && kkk == 1) {
                     //time(&start);
                    proverka_slova(slovas, st_str, preffix[i][0], preffix[i][0] + slo - 1, pos, strlen(slovas) - (y + yy), ukaz, mas_sl);
                    if (DOP == 0 && ukaz % 2 == 1) {
                        return 10;
                    };
                       //time(&end);
...
int main() {
   ...
   // printf("%s \n", player_bank_1);
    ...
    printf("ochki:::time person 1| %d ::: %.3f\n", Pl_1, (double)(p1) / CLOCKS_PER_SEC);
    printf("ochki:::time person 2| %d ::: %.3f\n", Pl_2, (double)(p2) / CLOCKS_PER_SEC);
    printf("k hodov 1: %d, 2: %d", res1, res2);
    FILE* i1 = fopen("player1.txt", "a+");
    fprintf(i1, " %.3f\n", (double)(p1) / CLOCKS_PER_SEC);
    fclose(i1);
    i1 = fopen("player2.txt", "a+");
    fprintf(i1, " %.3f\n", (double)(p2) / CLOCKS_PER_SEC);
    fclose(i1);
}